...
//	  val hc = new org.apache.spark.sql
	  ...
	  hc.sql("create table person(first_name string, last_name string, age int) row format delimited fields terminated by ','")
	  hc.sql("load data local inpath \"/home/projectone/nlfa\" into table person")
	  
	  val result = hc.sql("FROM person SELECT first_name,last_name, age")
